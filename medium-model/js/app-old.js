...
class QSTextureApp {
    ...
    excavationTerrain () {
        ...
        this.viewer.scene.globe.setExcavation(coordinations, this.underHeight, {
            ...
            // excavationBottomImageRepeat: new MEarth.Cartesian2(3.0, 3.0)
        });
    }
        ...
        try {
            ...
            const wallEntity = this.viewer.entities.add({
                ...
                wall: {
                    ...
                    minimumHeights: new Array(positions.length).fill((this.maxHeight - this.minHeight) * this.underScale * this.levelScale - this.underHeight),
    ...
    async createHeightLayerTexture (layer, minVal, maxVal, colorScheme, globalOpacity, levelScale) {
        try {
            ...
            // 计算高度（公里转换为米）
            // const heightInMeters = - (layer.height * this.underScale * levelScale);
            // debugger
            const heightInMeters = (this.maxHeight - layer.height) * this.underScale * levelScale - this.underHeight;
    ...
    calculateWallHeights (profile) {
        const maxHeightMeters = (this.maxHeight - this.minHeight) * this.underScale * this.levelScale - this.underHeight;
        return [- this.underHeight, - this.underHeight];
